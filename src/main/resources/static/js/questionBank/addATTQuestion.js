...
function newQuestionBlock(){
	var newBlockHtml = '<div class="questionBlock w3-panel w3-animate-top my-border"><div class="w3-black" style="margin-top: 16px;">'
						+ '<h4 style="padding-top:8px; padding-left:8px">Add Question <i>(Attachment Type)</i></h4>'
						...
						+ '</div>'
						+ '</div>'
						+ '<div class="w3-row" style="margin-top: 8px;"><label>Question:</label>'
						+ '<textarea></textarea>'
						+ '</div>'
						...
						+ '</p>'
						+ '</div>'
						+ '<div class="w3-container w3-half" style="padding-top: 24px;">'
						+ '<div class="w3-bar">'
						+ '<div class="w3-bar-item w3-right remove" id="editor'+blockCount+'"><i class="fa fa-trash" style="font-size:25px; cursor: pointer; color: red;"></i></div>'
						+ '</div>'
						+ '</div>'
						+ '</div>';
	...
	for(var i=0; i<=gvValue.length-1; i++){
		if(ckValue[i]==""){
			...
			bootbox.alert({
				size: 'medium',
				title: '<i class="fa fa-exclamation-circle" style="font-size: 25px; color: red;">&nbsp;&nbsp;Error</i>',
				message: 'Please select "Course Objective" of Question Number '+(i+1)+' on this page, to proceed further.'
			});
			ckValue=[];
			return;
		} else if(rfValue[i]==""){
