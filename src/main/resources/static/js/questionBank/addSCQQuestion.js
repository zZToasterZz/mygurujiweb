...
function saveClicked(){
	...
	if(qData == ""){
		...
		bootbox.alert({
			size: 'medium',
			title: '<i class="fa fa-exclamation-circle" style="font-size: 25px; color: red;">&nbsp;&nbsp;Error</i>',
			message: 'Single choice question must have four answer options.'
		});
	} else if(crtAns == null){
		bootbox.alert({
			size: 'medium',
		...
		opt1 = (normalizeString(opt1).slice(3)).slice(0,-4);
		opt2 = (normalizeString(opt2).slice(3)).slice(0,-4);
		opt3 = (normalizeString(opt3).slice(3)).slice(0,-4);
		opt4 = (normalizeString(opt4).slice(3)).slice(0,-4);*/
