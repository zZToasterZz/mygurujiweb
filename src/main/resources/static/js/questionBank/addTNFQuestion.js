...
function newQuestionBlock(){
	var newBlockHtml = '<div class="questionBlock w3-panel w3-animate-top my-border"><div class="w3-black" style="margin-top: 16px;">'
						+ '<h4 style="padding-top:8px; padding-left:8px">Add Question <i>(True or False Type)</i></h4>'
						+ '<label style="padding-left:8px; color: darkorange;">'+topicTitle+'</label></div>'
						...
						+ '<select class="w3-select w3-border" id="correctanseditor'+blockCount+'" name="selectCA">'
						...
						+ '</div>'
						+ '<div class="w3-bar">'
							+ '<div class="w3-bar-item w3-right remove" id="editor'+blockCount+'"><i class="fa fa-trash" style="font-size:25px; cursor: pointer; color: red;"></i>'
						+ '</div>'
						+ '</div>'
						+ '</div>';
...
function saveClicked(){
	...
	caValue = $.map($('select[name="selectCA"]'), function (e){
	...
	for(var i=0; i<=gvValue.length-1; i++){
		if(ckValue[i]==""){
			...
			bootbox.alert({
				size: 'medium',
				title: '<i class="fa fa-exclamation-circle" style="font-size: 25px; color: red;">&nbsp;&nbsp;Error</i>',
				message: 'Please select "Correct Answer" of Question Number '+(i+1)+' on this page, to proceed further.'
			});
			ckValue=[];
			return;
		}  else if(gvValue[i]==""){
